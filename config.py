...
def _get_module_name(name = None):
    for frameinfo in call_stack():
        ...
        if not filename.endswith('config.py'):
            ...
            try:
                modulename = path_sep.join(modulename.split(path_sep)[modulename.index(f'{path_sep}{name}{path_sep}'):])
            except ValueError:
...
def check_is_enable(name, module_name = None):
    ...
    if not config.get(module_name, False):
        raise RuntimeError(f'This module {name}.{module_name} is been called but is not enable in config')
...
def get_config(name, nargs=None, check_enable=True):
    ...
    try:
        ...
        all_config = {}
        logger.info('Because the configuration file does not exist, you are now running the configurator')
